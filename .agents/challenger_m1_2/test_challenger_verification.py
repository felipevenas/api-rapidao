...
async def test_not_found_route_envelope(client: AsyncClient):
    ...
    data = res.json()
    # Note: Checking if 404 returns {"status": "error", ...} or default FastAPI {"detail": "Not Found"}
...
async def test_method_not_allowed_route_envelope(client: AsyncClient):
    ...
    data = res.json()
    # Note: Checking if 405 returns {"status": "error", ...} or default FastAPI {"detail": "Method Not Allowed"}
